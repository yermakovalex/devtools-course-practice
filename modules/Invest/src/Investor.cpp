...
void Investor::help(const char* appname, const char* message) {
    message_ = std::string(message) +
        ...
        "<money* Expenses> <double RatePercent> <char* Function> [double RateParameter]\n\n";
}
...
bool Investor::validateNumberOfArguments(int argc,const char** argv) {
    if (argc == 1) {
        ...
        if (argc == 4 + year * 2 || (argc == 5 + year * 2 && !strcmp(argv[2 + year * 2 + 1],"FindNPV"))) {
            return true;
        }
        else {
...
std::string Investor::operator()(int argc,const  char** argv) {
    ...
    try {
        ...
        strcpy (args.Function,argv[2 + args.YearCount * 2 + 1]);
        if (strcmp(args.Function, "FindNPV") != 0 && strcmp(args.Function, "FindIRR") != 0 &&
            strcmp(args.Function, "FindRecoupmentTime") != 0 && strcmp(args.Function, "FindPBIndex") != 0) 
            throw std::string("Wrong Operation!\n");
        if (strcmp(args.Function, "FindNPV") ==  0 && argc == 5 + args.YearCount * 2)
            args.RateParameter = parseDouble(argv[argc-1]);
        else args.RateParameter = -1;
    }
    catch (std::string& str) {
        return str;
    }

    Investition invest(args.YearCount, args.Incomes, args.Expenses, args.RatePercent);
    std::ostringstream stream;
    if (!strcmp(args.Function, "FindNPV") && argc == 5 + args.YearCount * 2)
    {
